...
::BadExplodeS <- class extends Actor{
	...
	function run() {
		...
		if(gvPlayer != 0) {
			if(hitTest(shape, gvPlayer.shape)) gvPlayer.hurt = true
			if(floor(frame) <= 1 && distance2(x, y, gvPlayer.x, gvPlayer.y) < 64) {
				if(x < gvPlayer.x) gvPlayer.hspeed += 0.15
				if(x > gvPlayer.x) gvPlayer.hspeed -= 0.15
				if(y >= gvPlayer.y) gvPlayer.vspeed -= 0.15
			}
		}
		if(frame >= 1) {
				if(actor.rawin("WoodBlock")) foreach(i in actor["WoodBlock"]) {
...
::Burncap <- class extends Enemy {
		...
		if(active) {
			...
			if(!squish) {
				...
				if(frozen) {
					...
					//Draw
					if(smart) drawSpriteEx(sprBurncap, wrap(getFrames() / 12, 0, 3), floor(x - camx), floor(y - camy), 0, flip.tointeger(), 1, 1, 1)
					else drawSpriteEx(sprBurncap, wrap(getFrames() / 12, 0, 3), floor(x - camx), floor(y - camy), 0, flip.tointeger(), 1, 1, 1)
				}
			}
			else {
			newActor(Flame, x, y - 1)
	...
	function hurtfire() {
		deleteActor(id)
		newActor(BadExplode, x - 5, y)
		newActor(BadExplode, x + 5, y)
		newActor(BadExplode, x, y - 5)
		newActor(BadExplode, x, y + 5)
		newActor(Flame, x, y - 1)
	}
		...
		if(active) {
			if(!squish) {
				...
				if(frozen) {
					...
					//Draw
					drawSpriteEx(sprLivewire, wrap(getFrames() / 12, 0, 3), floor(x - camx), floor(y - camy), 0, flip.tointeger(), 1, 1, 1)
				}
			}
			else {
				squishTime++
